...
class Agent(object):
    ...
    def __init__(self, input_dim: int, output_dim: int, HIDDEN_DIM: int):
        self.dqn = DQN(input_dim, output_dim, HIDDEN_DIM).to(device)
    ...
    def get_action(self, X):
        NNinput = torch.autograd.Variable(torch.Tensor(X.reshape(-1, 4))).to(device)
        self.dqn.train(mode = False)
        qs = self.dqn(NNinput)
        _, argmax = torch.max(qs.data, 1)
        return int(argmax.to("cpu").numpy())
...
def train_helper(agent, minibatch):
    ...
    NNinput1 = torch.autograd.Variable(torch.Tensor(states.reshape(-1, 4))).to(device)
    NNinput2 = torch.autograd.Variable(torch.Tensor(next_states.reshape(-1, 4))).to(device)
    
    # Q-Value Updating
    Q_predict = agent.dqn(NNinput1)
    Q_target = Q_predict.clone().data.to("cpu").numpy()
    Q_target[np.arange(len(Q_target)), actions] = rewards + GAMMA * np.max(agent.dqn(NNinput2).data.to("cpu").numpy(), axis=1) * ~done
    Q_target = torch.autograd.Variable(torch.Tensor(Q_target)).to(device)
